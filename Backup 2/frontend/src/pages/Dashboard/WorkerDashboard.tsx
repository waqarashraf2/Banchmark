...
export default function WorkerDashboard() {
  ...
  const loadCritical = useCallback(async () => {
    try {
      ...
      // Don't overwrite manually started order
      setCurrentOrder(prev => {
        const apiOrder = currentRes.data.current_order || currentRes.data.order;

        if (!apiOrder) return prev;

        // prevent overwriting active order
        if (prev && prev.id === apiOrder.id) {
          return prev;
        }
      ...
      if ((isDrawer || isChecker || isQA) && results[2]) {
        const queueOrders = results[2].data?.orders || [];

        setRoleQueue(
          queueOrders.filter((o: Order) => {
              ...
              {doneOrders.map((order) => {
                ...
                return (
                  <div key={order.id} className="px-5 py-3 flex items-center justify-between">
                    <div>
                      <div className="text-sm font-medium text-slate-900">{isDrawer ? '••••••' : order.order_number}</div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      ...
                      {(user?.project?.id !== 16) && (
                        ...
                        <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600">Drawer</th>
                        {isQA && (
...
{(user?.project?.id !== 16) && (
                  ...
                  >
                    ...
                    <div className="p-5">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {isDrawer ? 'Open Work Form' : isQA ? 'Open QA Review' : 'Open Check Form'}
                        </Button>
    ...
    {order.created_at
      ...
      : '—'}
                              ...
                              <td className="px-4 py-3">
                                <span className="font-semibold text-slate-900">{isDrawer ? '••••••' : order.order_number}</span>
                              </td>
                              {(user?.project?.id !== 16) && (
