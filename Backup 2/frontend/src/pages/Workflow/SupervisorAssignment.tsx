...
export default function SupervisorAssignment() {
  ...
  type AssignmentRoleColumn = {
    ...
    label: string;
    width?: string;
    role: 'drawer' | 'checker' | 'file_uploader' | 'qa';
  };

  /* Project time clock */
  const [projectTz, setProjectTz] = useState('Asia/Karachi');
  ...
  const showClientSummaryCard = projectId === 8 || projectId === 7 || projectId === 9 || projectId === 11 || projectId === 46;
  const effectiveProjectTz = isProject16 ? 'Asia/Ho_Chi_Minh' : 'Asia/Karachi';
  ...
  const loadData = useCallback(async (_page = 1, isRefresh = false) => {
    ...
    try {
      ...
      setProjectId(d.project?.id ?? null);
  ...
  }, [effectiveProjectTz, selectedQueue, statusFilter, searchQuery, startDate, endDate, selectedWorker]);

  useEffect(() => { loadData(1); }, [loadData]);
  useEffect(() => { setProjectTz(effectiveProjectTz); }, [effectiveProjectTz]);
  ...
  /* Sort orders by priority: rush/urgent, then high, then normal/low */
  const priorityWeight: Record<string, number> = { rush: 0, urgent: 0, high: 1, normal: 2, low: 3 };
  const sortedOrders = useMemo(() =>
    [...displayedOrders].sort((a, b) => (priorityWeight[a.priority] ?? 2) - (priorityWeight[b.priority] ?? 2)),
    [displayedOrders]);
  ...
  const currentProjectDate = useMemo(() => {
    return getProjectDateValue(effectiveProjectTz);
  }, [effectiveProjectTz]);
  ...
  const parseDueIn = useCallback((raw: string | null, receivedAt?: string | null): number | null => {
    const getProjectNow = () => {
      const projectNowStr = new Date().toLocaleString('en-US', { timeZone: effectiveProjectTz });
  ...
  }, [isProject16, selectedQueue, showCodeQueues]);
  ...
  }, [defaultPrimaryColumns, fixedTrailingFields, isProject16, projectColumns, selectedQueue]);
  ...
  const visibleRoleColumns = useMemo<AssignmentRoleColumn[]>(() => {
    const roleColumnMap: Record<AssignmentRoleColumn['key'], AssignmentRoleColumn> = {
      drawer_name: { key: 'drawer_name', label: 'Drawer', width: '13%', role: 'drawer' },
      checker_name: { key: 'checker_name', label: 'Checker', width: '13%', role: 'checker' },
      file_uploader_name: { key: 'file_uploader_name', label: 'Filler', width: '13%', role: 'file_uploader' },
      ...
      case 'received_at':
        return (
          <td className="px-3 py-2 text-slate-600">
            {order.received_at ? new Date(order.received_at).toLocaleString('en-GB', {
              day: '2-digit',
              month: 'short',
              hour: '2-digit',
              minute: '2-digit',
                        ...
                        {sortedOrders.map((o, idx) => (
                          <motion.tr key={o.id}
                            ...
                            {visibleRoleColumns.map((column) => {
                              ...
                              if (column.role === 'file_uploader') {
                                return (
                                  <td key={`${o.id}-${column.key}`} className="px-3 py-2 text-slate-700">
                                    {o.file_uploader_name || '-'}
                                  </td>
                                );
                              }
