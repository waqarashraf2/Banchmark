...
protected function parseDueIn(string $dueRaw): string
{
    ...
    public function run()
    {
        ...
        while ($page <= $this->maxPages) {
            ...
            try {
                $pageUrl = env('EXTERNAL_PORTAL_URL') . (str_contains(env('EXTERNAL_PORTAL_URL'), '?') ? '&' : '?') . 'page=' . $page;
                ...
                for ($i=1; $i<$rows->length; $i++) {
                    ...
                    $dueIn = $this->parseDueIn($dueInRaw, $receivedAt);

                    // Fetch variant safely
                    try {
                ...
                if (!empty($records)) {
                    ...
                    DB::table('project_15_orders')->upsert(
                        $records,
                        ['order_number'],
